package com.wheel.daniel.downloder;

import android.content.Context;

import com.wheel.daniel.downloder.callback.DownloadCallback;
import com.wheel.daniel.downloder.data.DownloadInfo;
import com.wheel.daniel.downloder.net.FileTask;
import com.wheel.daniel.downloder.net.UpDataHandler;
import com.wheel.daniel.downloder.utils.ThreadPool;
...
public class DownloadManger {
    ...
    private Map<String, UpDataHandler> progressHandlerMap = new HashMap<>();//保存任务的进度处理对象
    private Map<String, DownloadInfo> downloadDataMap = new HashMap<>();//保存任务数据
    ...
    public void resume(String url) {
        if (progressHandlerMap.containsKey(url) &&
                (progressHandlerMap.get(url).getCurrentState() == PAUSE || progressHandlerMap.get(url).getCurrentState() == ERROR)) {
    ...
    private synchronized void execute(DownloadInfo downloadData, DownloadCallback callback) {
        ...
        UpDataHandler downloadProgressHandler = new UpDataHandler(mContext, downloadData, callback);
        FileTask fileTask = new FileTask(mContext, downloadData, downloadProgressHandler.getHandler());
     ...
     */
    public synchronized void setOnDownloadCallback(DownloadInfo downloadData, DownloadCallback downloadCallback) {
        downloadDataMap.put(downloadData.getUrl(), downloadData);
